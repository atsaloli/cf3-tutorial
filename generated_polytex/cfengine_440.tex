...
\begin{itemize}
\item{} hosts in the London data center,
\item{} Solaris hosts in Texas,
\item{} Linux hosts between 2:00 and 4:00 A.M. on Sunday
\end{itemize}
...
\subsection{Note: Classes have Scope}
...
\subsection{Methods provide encapsulation of multiple issues}
...
\begin{itemize}
\item{} Write a policy that has two bundles.

\begin{itemize}
\item{} The first bundle does something visible (such as a reports type promise that says ``bundle1'') AND calls the second bundle.
\item{} The second bundle reports ``bundle2''.
\end{itemize}
\end{itemize}
...
\end{verbatim}
\item{} add parsing of argument to the bundle (if ``on'', then do the above;
...
\begin{enumerate}
\item{} Write a bundle ``webserver'' that will ensure an Apache httpd package
...
\begin{enumerate}
\item{} Make sure the service is off when the argument is ``off''.
\end{enumerate}
